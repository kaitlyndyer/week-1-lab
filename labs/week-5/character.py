...
class Character(ABC):
    ...
    def collect_coin(self):
        ...
        if self.coins >= 100:
            self.coins = 0
            self.lives += 1
            return f"{self.name} collected 100 coins! Extra life! ({self.lives} lives)"
        return f"{self.name} collected a coin! ({self.coins}/100)"
    ...
    def take_damage(self):
        ...
        self.lives -= 1
        if self.is_alive:
            return f"{self.name} was hit! {self.lives} lives remaining!"
        else:
            return f"{self.name} was hit! Game over for {self.name}!"
